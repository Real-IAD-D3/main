...
class Features(torch.nn.Module):
    ...
    def __init__(self, args, image_size=224, f_coreset=0.1, coreset_eps=0.9):
        ...
        if self.args.use_uff:
            if self.args.xyz_backbone_name == 'FPFH' :
                self.fusion = FeatureFusionBlock(33, 768, mlp_ratio=4.)
            else :
                self.fusion = FeatureFusionBlock(1152, 768, mlp_ratio=4.)
            #应为没找到预训练的所以先注释掉
            ckpt = torch.load('/fuxi_team14_intern/m3dm/checkpoints/uff_pretrain.pth')['model']
            #ckpt = torch.load('/workspace/data3/code/M3DM-main/checkpoints5/checkpoint-99.pth')['model']
            #ckpt = torch.load('/workspace/data3/code/M3DM-main/checkpoints6/checkpoint-400.pth')['model']
    ...
    def calculate_metrics(self):
        ...
        self.pixel_preds = np.array(self.pixel_preds)
            # 在计算metrics之前检查和处理NaN值
        if np.any(np.isnan(self.image_preds)):
            print(f"Warning: Found {np.sum(np.isnan(self.image_preds))} NaN values in image_preds")
            # 将NaN替换为0或该特征的平均值
            self.image_preds = np.nan_to_num(self.image_preds, nan=0.0)
        
        if np.any(np.isnan(self.pixel_preds)):
            print(f"Warning: Found {np.sum(np.isnan(self.pixel_preds))} NaN values in pixel_preds")
            self.pixel_preds = np.nan_to_num(self.pixel_preds, nan=0.0)
        
        try:
            self.image_rocauc = roc_auc_score(self.image_labels, self.image_preds)
        except Exception as e:
            print(f"Error calculating image_rocauc: {e}")
            print(f"image_labels shape: {self.image_labels.shape}")
            print(f"image_preds shape: {self.image_preds.shape}")
            print(f"image_labels unique values: {np.unique(self.image_labels)}")
            print(f"image_preds range: [{np.min(self.image_preds)}, {np.max(self.image_preds)}]")
            self.image_rocauc = 0.0
        #self.image_rocauc = roc_auc_score(self.image_labels, self.image_preds)
    ...
    def run_late_fusion(self):
        ...
        #         # 检查并处理缺失值
        # if torch.isnan(self.s_lib).any():
        #     print("警告: self.s_lib 中包含缺失值，正在进行处理。")
        #     self.s_lib = self.s_lib[~torch.isnan(self.s_lib).any(dim=1)]  # 删除包含缺失值的样本

        # if torch.isnan(self.s_map_lib).any():
        #     print("警告: self.s_map_lib 中包含缺失值，正在进行处理。")
        #     self.s_map_lib = self.s_map_lib[~torch.isnan(self.s_map_lib).any(dim=1)]  # 删除包含缺失值的样本
